...
void msgProcess(String lightCmd)
{
  ...
  String rec_cs = lightCmd.substring(0, idx);
  lightCmd = lightCmd.substring(idx);
  Bridge.print("cmd: ");
  Bridge.println(lightCmd);
  Bridge.print("rec_cs: ");
  Bridge.println(rec_cs);
  Bridge.print("cal_cs: ");
  Bridge.println(checksum(lightCmd));
  ...
  if (topic_name.compareTo("control") == 0)
  {
    ...
    if (angular_y > 2)
    {
      Radio.print(micros() - lastMsgProcess);
      Radio.print("    ALERT !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!    yaw = ");
      Radio.println(angular_y);
      Radio.print(lightCmd);
      return;
    }
